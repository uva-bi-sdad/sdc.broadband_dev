...
frames <- vector("list", i%/% increment)


for(i in seq(from=1, to=10, by=increment)) { # reverse geocode has limit of 100
  pb$tick()
  frame <- combined[i:(i+(increment-1)),]
  # print(frame)
  reverse <- frame[c("lat", "lon")] %>%
  reverse_geocode(lat = lat, long = lon, method = 'osm',
                address = address, full_results = FALSE)
  # reverse <- apply(reverse, 2, as.character)
  print(reverse)
  frames[[(i%/% increment + 1)]] <- reverse
}
```
```{r}
frames[[1]]
